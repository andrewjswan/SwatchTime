"""Adds config flow for Swatch Time integration."""
...
from .sensor import SwatchTimeSensor
...
class SwatchTimeConfigFlowHandler(SchemaConfigFlowHandler, domain=DOMAIN):
    """Handle a config flow for Swatch Time."""

    config_flow = CONFIG_FLOW

    def async_config_entry_title(self, options: Mapping[str, Any]) -> str:
        """Return config entry title."""
        return f"Swatch Time {options[CONF_DISPLAY_OPTIONS]}"
...
@websocket_api.websocket_command(
    {
        vol.Required("type"): "swatch_time/start_preview",
...
async def ws_start_preview(
    ...
    preview_entity = SwatchTimeSensor(validated["swatch_time"])
